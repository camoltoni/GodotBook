...
func _ready():
	...
	for cell in $Walls.get_used_cells_by_id(door_id):
		doors.append(cell)
	_spawn_items()
	assert($Player.connect("dead", self, "game_over") == OK)
	assert($Player.connect("grabbed_key", self, "_on_Player_grabbed_key") == OK)
	assert($Player.connect("win", self, "_on_Player_win") == OK)
